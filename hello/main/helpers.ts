...
export async function submitScriptFunctionTransaction(
  ...
  var payload = {
    ...
    "expiration_timestamp_secs": BigInt(99999999999).toString(),
    "payload": {
  ...
  const signingMessage = await generateScriptFunctionTransactionSigningMessage(payload);
  const signatureTmp = await ed.sign(signingMessage, privateKeyBytes);
...
export async function generateScriptFunctionTransactionSigningMessage(
    payload: {},
) {
  const settings = {
    method: "POST",
  ...
  const res = await fetch(relativeUrl(`/transactions/signing_message`), settings);
  return await res.json();
}
