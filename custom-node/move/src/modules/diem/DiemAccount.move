...
//    }
    ...
    spec withdraw_from_balance {
        include WithdrawFromBalanceAbortsIf<Token>{payee: _payee};
    ...
    fun withdraw_from<Token>(
        ...
        amount: u64,
        metadata: vector<u8>,
    ): Diem<Token> acquires Balance, DiemAccount {
        DiemTimestamp::assert_operating();
    ...
    public fun create_regular_account<Token>(
...
//        );
    ...
    spec prologue_common {
        let transaction_sender = Signer::spec_address_of(sender);
        let max_transaction_fee = _txn_gas_price * _txn_max_gas_units;
        include PrologueCommonAbortsIf<Token> {
            transaction_sender,
            max_transaction_fee,
        };
    ...
    ) acquires DiemWriteSetManager, DiemAccount {
