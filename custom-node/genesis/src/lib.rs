...
use diem_genesis_tool::swarm_config::SwarmConfig;
#[cfg(test)]
...
const MOVE_BYTECODE_DIR: &str =
    "../move/storage/0x00000000000000000000000000000001/modules";

pub fn generate_swarm_config(node_config_dir: &Path) -> Result<SwarmConfig> {
    assert!(
    ...
    diem_genesis_tool::swarm_config::SwarmConfig::build(&builder, &node_config_dir)
}

/// Generate a NodeConfig for a single validator node
pub fn generate_custom_node_config(node_config_dir: &Path) -> Result<NodeConfig> {
    let test_config = generate_swarm_config(node_config_dir)?;
    Ok(NodeConfig::load(test_config.config_files[0].clone())?)
}
